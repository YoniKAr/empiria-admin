...
import { adminIssueTickets, adminSendTicketsToEmail } from './actions';
...
export function IssueTicketsModal({ eventId, tiers }: IssueTicketsModalProps) {
  ...
  function handleSubmit() {
    ...
    startTransition(async () => {
      const result = await adminIssueTickets({
        eventId,
        tierId,
        quantity,
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
            <h3 className="text-lg font-bold text-slate-900 mb-4">Issue Tickets Manually</h3>

            <label className="block text-sm font-medium text-slate-700 mb-1">Ticket Tier</label>
            <select
              value={tierId}
            ...
            {selectedTier && selectedTier.price > 0 && (
              <div className="flex items-center gap-3 mb-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  />
                  <span className="text-sm text-slate-700">Free (complimentary)</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="radio"
